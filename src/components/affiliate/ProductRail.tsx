...
interface Props {
  ...
  title?: string
  limit?: number
  /** EPNE customid, e.g. team-kansas-city-chiefs */
  customid?: string
}

/**
 * Server-rendered eBay rail. Homepage/search already fetch on the server so
 * crawlers see affiliate links; team/league/event pages used to fetch only
 * in the browser and looked empty to curl/Google ("Live listings" theater).
 * Hide the heading entirely when the query returns nothing.
 */
export default async function ProductRail({ query, title, limit = 12, customid }: Props) {
  let products: EbayProduct[] = []
  try {
    products = await searchEbayProducts(query, limit, customid ? { customid } : {})
  } catch {
    products = []
  }
